...
function mc_file_cache( $buffer, $flags ) {
	...
	if ( ! file_exists( $cache_dir ) ) {
		...
		$buffer = apply_filters( 'mc_pre_cache_buffer', $buffer );
	}
...
function mc_serve_file_cache() {
	$cache_dir = ( defined( 'MC_CACHE_DIR' ) ) ? rtrim( MC_CACHE_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/cache/simple-cache';
	...
	$html_path   = $cache_dir . '/' . rtrim( mc_get_url_path(), '/' ) . '/' . $file_name . 'html';
	$json_path   = $cache_dir . '/' . rtrim( mc_get_url_path(), '/' ) . '/' . $file_name . 'json';
	$header_path = $cache_dir . '/' . rtrim( mc_get_url_path(), '/' ) . '/headers.json';
 ...
 * @return string
 */
function mc_get_cache_dir() {
	return ( defined( 'MC_CACHE_DIR' ) ) ? rtrim( MC_CACHE_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/cache/simple-cache';
}
 ...
 * @return string
 */
function mc_get_config_dir() {
	return ( defined( 'MC_CONFIG_DIR' ) ) ? rtrim( MC_CONFIG_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/mc-config';
}
 ...
 * @return string
 */
function mc_get_config_file_name() {
 ...
 * @return bool|array
 */
function mc_load_config() {
	if ( @file_exists( mc_get_config_dir() . '/config-network.php' ) ) {
		return include mc_get_config_dir() . '/config-network.php';
	} elseif ( @file_exists( mc_get_config_dir() . '/' . mc_get_config_file_name() ) ) {
		return include mc_get_config_dir() . '/' . mc_get_config_file_name();
	}
 ...
 * @return boolean
 */
function mc_url_exception_match( $exception, $regex = false ) {
